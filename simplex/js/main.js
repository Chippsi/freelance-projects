...
/* const header = {
	...
	init(mediaQueries) {
		this.paddingFix();
		mediaQueries.forEach(mediaData => {
			let mediaQ = window.matchMedia(('(min-width:' + mediaData + 'px)'));
			mediaQ.addListener(this.paddingFix);
		});
	},
}
header.init([1401, 991, 767]); */
	...
	for (let index = 0; index < spollers.length; index++) {
		...
		spoller.addEventListener("click", function (e) {
			if (spollersGo) {
				...
				/* console.log(window.getComputedStyle(spoller.nextElementSibling).display); */
...
let mobileUslugi = document.querySelector(".header__mobile-uslugi");
if (mobileUslugi != null) {
	let delay = 500;
	let spollersWrp = document.querySelector(".home__spollers-wrp");
	let sidebar = document.querySelector('.home__sidebar');
	let sidebarCLose = document.querySelector('.home__sidebar-close');
	mobileUslugi.addEventListener("click", function (e) {
		if (unlock) {
			body_lock(delay);
			mobileUslugi.classList.toggle("_active");
			spollersWrp.classList.toggle("_active");
	...
	sidebar.addEventListener('click', (e)=>{
		if (e.target === e.currentTarget) {
            body_lock_remove(delay)
			mobileUslugi.classList.remove("_active");
			spollersWrp.classList.remove("_active");
